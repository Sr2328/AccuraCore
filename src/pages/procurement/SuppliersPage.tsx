...
import {
  ...
  CheckCircle2, XCircle, Download, Users, UserCheck, UserX,
} from "lucide-react";
...
type Supplier = {
  ...
  email: string | null; supplier_type: SupplierType; process_types: string[] | null;
  material_categories: string[] | null; gst_number: string | null; pan_number: string | null;
  bank_name: string | null; bank_account: string | null; bank_ifsc: string | null;
  address: string | null; city: string | null; state: string | null; rating: number | null;
...
══════════════════════════════════════════════ */
const JW_PROCESSES = [
  "Polishing", "Hardening", "CG (Surface Grinding)", "Laser Welding", "CMM (Coordinate Measuring)",
  "Vacuum Heat Treatment", "EDM (Wire Cut)", "EDM (Die Sinking)", "CNC Milling", "CNC Turning",
  "Heat Treatment", "Chrome Plating", "Nitriding", "Black Oxide", "Other",
];
const RM_CATEGORIES = [
  "Steel (Tool Steel)", "Steel (Mild Steel)", "Steel (Stainless Steel)", "Aluminium", "Copper",
  "Brass", "Beryllium Copper", "Hot Work Steel", "Cold Work Steel", "Pre-hardened Steel", "Other Metals",
];
const CONSUMABLE_CATEGORIES = [
  "Cutting Tools", "Abrasives & Grinding Wheels", "Lubricants & Coolants", "Safety Equipment",
  "PPE (Gloves, Masks, Goggles)", "Cleaning Supplies", "Stationery & Office", "Electrical Components",
  "Fasteners & Hardware", "Hydraulic & Pneumatic", "Packaging Materials", "Other Consumables",
];
...
const TAB_CONFIG: Record<SupplierType, {
  ...
  job_work: {
    label: "Job Work Suppliers", shortLabel: "Job Work", icon: Wrench,
    color: "text-violet-600 dark:text-violet-400",
    bg: "bg-violet-50 dark:bg-violet-900/20",
    border: "border-violet-100 dark:border-violet-900/40",
    activeBg: "bg-violet-600", activeBorder: "border-violet-600",
    activeText: "text-white", accent: "violet",
    description: "Machining & process work suppliers",
  },
  raw_material: {
    label: "Raw Material Suppliers", shortLabel: "Raw Material", icon: Package,
    color: "text-blue-600 dark:text-blue-400",
    bg: "bg-blue-50 dark:bg-blue-900/20",
    border: "border-blue-100 dark:border-blue-900/40",
    activeBg: "bg-blue-600", activeBorder: "border-blue-600",
    activeText: "text-white", accent: "blue",
    description: "Steel, copper, aluminium and base materials",
  },
  consumable: {
    label: "Consumables & Others", shortLabel: "Consumables", icon: ShoppingCart,
    color: "text-emerald-600 dark:text-emerald-400",
    bg: "bg-emerald-50 dark:bg-emerald-900/20",
    border: "border-emerald-100 dark:border-emerald-900/40",
    activeBg: "bg-emerald-600", activeBorder: "border-emerald-600",
    activeText: "text-white", accent: "emerald",
    description: "Tools, safety items & consumable goods",
  },
};
...
const inputCls = "w-full px-3 py-2 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm text-neutral-800 dark:text-white placeholder:text-neutral-400 dark:placeholder:text-neutral-600 focus:outline-none focus:border-[#EAB308]/60 transition-colors";
...
const MultiSelect = ({ label, options, selected, onChange, color }: {
  ...
  const toggle = (o: string) =>
    selected.includes(o) ? onChange(selected.filter(s => s !== o)) : onChange([...selected, o]);
  return (
    <div>
...
const Modal = ({ title, onClose, children, footer, wide }: {
  ...
  <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
    ...
    <div className={cn(
      "relative z-10 w-full bg-white dark:bg-neutral-900 rounded-t-2xl sm:rounded-xl border border-neutral-100 dark:border-neutral-800 shadow-2xl max-h-[92vh] flex flex-col",
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100 dark:border-neutral-800 flex-shrink-0">
        ...
        <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors text-neutral-400">
          <X className="w-4 h-4" />
        </button>
      </div>
...
const DR = ({ label, value, icon: Icon }: { label: string; value?: React.ReactNode; icon?: React.ElementType }) => (
...
/* Sharper SC card with shadow */
const SC = ({ title, icon: Icon, children }: { title: string; icon?: React.ElementType; children: React.ReactNode }) => (
  <div className="rounded-lg bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-md overflow-hidden">
    <div className="flex items-center gap-2 px-4 py-3 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800/60">
...
const TagPill = ({ label, color }: { label: string; color: string }) => (
  <span className={cn("inline-flex items-center px-2.5 py-1 rounded border text-xs font-medium", color)}>{label}</span>
);

const Stars = ({ rating }: { rating: number | null }) => (
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map(n => (
      <Star key={n} className={cn("w-3.5 h-3.5", n <= (rating ?? 0) ? "text-[#EAB308]" : "text-neutral-200 dark:text-neutral-700")}
...
type FormData = {
  ...
  gst_number: string; pan_number: string; bank_name: string;
  bank_account: string; bank_ifsc: string; address: string;
  city: string; state: string; rating: number; payment_terms: string; notes: string;
};
const defaultForm = (): FormData => ({
  name: "", contact_person: "", phone: "", email: "",
  process_types: [], material_categories: [],
  gst_number: "", pan_number: "", bank_name: "", bank_account: "", bank_ifsc: "",
  address: "", city: "", state: "", rating: 3, payment_terms: "30 Days", notes: "",
});

const SupplierFormModal = ({ supplierType, supplier, onClose }: {
  ...
  const [form, setForm] = useState<FormData>(() => ({
    ...
    process_types: supplier?.process_types ?? [], material_categories: supplier?.material_categories ?? [],
    gst_number: supplier?.gst_number ?? "", pan_number: supplier?.pan_number ?? "",
    bank_name: supplier?.bank_name ?? "", bank_account: supplier?.bank_account ?? "",
    bank_ifsc: supplier?.bank_ifsc ?? "", address: supplier?.address ?? "",
    city: supplier?.city ?? "", state: supplier?.state ?? "",
    rating: supplier?.rating ?? 3, payment_terms: supplier?.payment_terms ?? "30 Days",
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        phone: form.phone || null, email: form.email || null, supplier_type: supplierType,
        ...
        bank_ifsc: form.bank_ifsc || null, address: form.address || null,
        city: form.city || null, state: form.state || null, rating: form.rating,
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["suppliers", supplierType] }); onClose(true); },
    onError: (e: any) => setError(e.message ?? "Failed to save"),
  });
  const multiOptions = supplierType === "job_work" ? JW_PROCESSES : supplierType === "raw_material" ? RM_CATEGORIES : CONSUMABLE_CATEGORIES;
  const multiColor = supplierType === "job_work" ? "bg-violet-600" : supplierType === "raw_material" ? "bg-blue-600" : "bg-emerald-600";
  return (
    <Modal title={isEdit ? "Edit Supplier" : `Add ${cfg.shortLabel} Supplier`} onClose={() => onClose()} wide
      footer={
        <div className="flex gap-3">
          <button onClick={() => onClose()} className="flex-1 py-2.5 rounded-lg border border-neutral-200 dark:border-neutral-700 text-sm font-medium text-neutral-600 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors">Cancel</button>
          <button onClick={() => mutation.mutate()} disabled={mutation.isPending}
            className="flex-1 py-2.5 rounded-lg bg-[#EAB308] hover:bg-yellow-500 text-black font-semibold text-sm transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
            {mutation.isPending ? <><Loader2 className="w-4 h-4 animate-spin" />{isEdit ? "Saving…" : "Adding…"}</> : isEdit ? "Save Changes" : "Add Supplier"}
          </button>
        </div>
      }>
      <div className={cn("flex items-center gap-2.5 px-3 py-2.5 rounded-lg border", cfg.bg, cfg.border)}>
      ...
      {error && (
        <div className="flex items-center gap-2 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/40 rounded-lg px-3 py-2.5">
      ...
      <MultiSelect label={supplierType === "job_work" ? "Work Processes Offered" : "Material / Product Categories"}
...
══════════════════════════════════════════════ */
const SupplierDetail = ({ supplier, onBack, onEdit }: { supplier: Supplier; onBack: () => void; onEdit: () => void }) => {
  const qc = useQueryClient();
  const cfg = TAB_CONFIG[supplier.supplier_type];
  const toggleActive = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("suppliers").update({ is_active: !supplier.is_active, updated_at: new Date().toISOString() }).eq("id", supplier.id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["suppliers", supplier.supplier_type] }); onBack(); },
  });
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="flex items-start gap-3">
        <button onClick={onBack}
          className="w-9 h-9 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 flex items-center justify-center hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors flex-shrink-0 mt-0.5">
          <ArrowLeft className="w-4 h-4 text-neutral-500 dark:text-neutral-400" strokeWidth={1.8} />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-xl font-bold text-neutral-900 dark:text-white truncate">{supplier.name}</h2>
            <span className={cn("inline-flex items-center px-2 py-0.5 rounded border text-xs font-semibold", cfg.bg, cfg.border, cfg.color)}>
              <cfg.icon className="w-3 h-3 mr-1" strokeWidth={2} />{cfg.shortLabel}
            </span>
            {!supplier.is_active && (
              <span className="inline-flex items-center px-2 py-0.5 rounded border text-xs font-semibold bg-red-50 dark:bg-red-900/20 text-red-500 border-red-100">Inactive</span>
            )}
          </div>
        ...
        <div className="flex gap-2 flex-shrink-0">
          <button onClick={onEdit} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 hover:bg-neutral-50 dark:hover:bg-neutral-800 text-sm font-medium text-neutral-600 dark:text-neutral-400 transition-colors">
          ...
          <button onClick={() => toggleActive.mutate()} disabled={toggleActive.isPending}
            className={cn("inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm font-medium transition-colors",
              supplier.is_active
                ? "border-red-100 dark:border-red-900/40 bg-red-50 dark:bg-red-900/15 text-red-600 dark:text-red-400 hover:bg-red-100"
                : "border-emerald-100 bg-emerald-50 text-emerald-600 hover:bg-emerald-100")}>
            {toggleActive.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
              : supplier.is_active ? <XCircle className="w-3.5 h-3.5" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
            <span className="hidden sm:inline">{supplier.is_active ? "Deactivate" : "Activate"}</span>
          </button>
        </div>
      </div>
      {/* Stats row */}
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg shadow-md px-4 py-4 text-center">
          <p className="text-2xl font-bold text-neutral-900 dark:text-white tabular-nums">{jobCount}</p>
          <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1">{supplier.supplier_type === "job_work" ? "Job Work Orders" : "Purchase Orders"}</p>
        </div>
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg shadow-md px-4 py-4 text-center flex flex-col items-center justify-center gap-1">
          <Stars rating={supplier.rating} />
          <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">Rating</p>
        </div>
        <div className={cn("rounded-lg px-4 py-4 text-center border shadow-md", cfg.bg, cfg.border)}>
          <p className={cn("text-2xl font-bold tabular-nums", cfg.color)}>{tags.length}</p>
          <p className={cn("text-xs mt-1 opacity-80", cfg.color)}>{supplier.supplier_type === "job_work" ? "Processes" : "Categories"}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          {tags.length > 0 && (
            <SC title={supplier.supplier_type === "job_work" ? "Work Processes" : "Material / Product Categories"}
              ...
              <div className="flex flex-wrap gap-2">{tags.map(t => <TagPill key={t} label={t} color={tagColor} />)}</div>
            </SC>
          )}
          <SC title="Contact Information" icon={Phone}>
            ...
            {(supplier.address || supplier.city) && <DR label="Address" icon={MapPin} value={[supplier.address, supplier.city, supplier.state].filter(Boolean).join(", ")} />}
          </SC>
          {supplier.notes && (
            <div className="rounded-lg bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 shadow-sm px-4 py-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <SC title="Record Info" icon={FileText}>
            ...
            <DR label="Status" value={
              <span className={cn("text-xs font-semibold", supplier.is_active ? "text-emerald-600 dark:text-emerald-400" : "text-red-500")}>
...
const SupplierCard = ({ supplier, onClick }: { supplier: Supplier; onClick: () => void }) => {
  ...
  return (
    <div onClick={onClick}
      className="flex items-center gap-4 px-4 sm:px-5 py-4 hover:bg-neutral-50 dark:hover:bg-neutral-800/40 cursor-pointer transition-colors border-b border-neutral-100 dark:border-neutral-800 last:border-0 group">
      {/* Avatar */}
      <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-base border shadow-sm", cfg.bg, cfg.border, cfg.color)}>
        ...
        {tags.length > 0 && (
          <div className="flex gap-1.5 mt-1.5 flex-wrap">
            {tags.slice(0, 2).map(t => (
              <span key={t} className={cn("inline-flex items-center px-2 py-0.5 rounded border text-[10px] font-medium", tagColor)}>{t}</span>
            ))}
            {tags.length > 2 && <span className="text-[10px] text-neutral-400 dark:text-neutral-500 self-center">+{tags.length - 2} more</span>}
          </div>
        )}
      </div>
      {/* Rating */}
      <div className="hidden sm:flex flex-col items-end gap-1 flex-shrink-0">
        <Stars rating={supplier.rating} />
        {supplier.payment_terms && <p className="text-xs text-neutral-400 dark:text-neutral-500">{supplier.payment_terms}</p>}
      </div>
...
/* ══════════════════════════════════════════════
   TAB PANEL
══════════════════════════════════════════════ */
const TabPanel = ({ supplierType, onSelect }: { supplierType: SupplierType; onSelect: (s: Supplier) => void }) => {
  const cfg = TAB_CONFIG[supplierType];
  const [search, setSearch] = useState("");
  ...
  const { data: suppliers = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("suppliers").select("*").eq("supplier_type", supplierType).order("name");
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder={supplierType === "job_work" ? "Search name, process, city…" : "Search name, material, city…"}
            className="w-full pl-9 pr-8 py-2.5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg text-sm text-neutral-800 dark:text-white placeholder:text-neutral-300 dark:placeholder:text-neutral-600 focus:outline-none focus:border-[#EAB308]/60 transition-colors shadow-sm" />
          {search && (
        ...
        <label className="flex items-center gap-2 px-3 py-2.5 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 cursor-pointer hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors shadow-sm">
          <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} className="w-4 h-4 rounded accent-[#EAB308]" />
      ...
      <div className="rounded-lg bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-md overflow-hidden">
        {/* Column header */}
        <div className="hidden md:flex items-center gap-4 px-5 py-2.5 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800/40">
          <span className="flex-1 text-xs font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">Supplier</span>
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3 border", cfg.bg, cfg.border)}>
            ...
            {!search && (
              <button onClick={() => setShowAdd(true)}
                className="mt-4 inline-flex items-center gap-1.5 bg-[#EAB308] hover:bg-yellow-500 text-black text-sm font-semibold px-4 py-2 rounded-lg transition-colors">
        ...
        {filtered.length > 0 && (
          <div className="px-5 py-3 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-800/20">
            <p className="text-xs text-neutral-400 dark:text-neutral-500">{filtered.length} of {suppliers.length} suppliers</p>
          </div>
        )}
      </div>
      {showAdd && <SupplierFormModal supplierType={supplierType} onClose={() => setShowAdd(false)} />}
    </>
  );
};
...
export default function SuppliersPage() {
  ...
  const { data: allSuppliers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("suppliers").select("supplier_type, is_active");
      return data ?? [];
    },
  });

  const totalActive = allSuppliers.filter((s: any) => s.is_active).length;
  const totalInactive = allSuppliers.filter((s: any) => !s.is_active).length;
  const counts: Record<string, number> = {};
  (allSuppliers as any[]).forEach((s: any) => {
    if (s.is_active) counts[s.supplier_type] = (counts[s.supplier_type] ?? 0) + 1;
  });
  ...
  if (selected) {
    return (
      <>
        <SupplierDetail supplier={selected} onBack={() => setSelected(null)} onEdit={() => setEditing(selected)} />
        {editing && (
          <SupplierFormModal supplierType={editing.supplier_type} supplier={editing}
            onClose={() => {
              setEditing(null);
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-400 dark:text-neutral-500 mt-0.5">
            {totalActive + totalInactive} total · {totalActive} active
          </p>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <button className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 hover:bg-neutral-50 dark:hover:bg-neutral-800 text-sm font-medium text-neutral-600 dark:text-neutral-400 transition-colors shadow-sm">
            <Download className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Export PDF</span>
          </button>
          <button onClick={() => setShowAdd(true)}
            className="inline-flex items-center gap-2 bg-[#EAB308] hover:bg-yellow-500 active:scale-[.98] text-black font-semibold text-sm px-4 py-2 rounded-lg transition-all shadow-sm">
            <Plus className="w-4 h-4" strokeWidth={2.5} />Add
          </button>
        </div>
      </div>

      {/* ─── HEADER CARDS (wireframe layout) ────────── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">

        {/* LEFT — big card: supplier type list */}
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg shadow-md overflow-hidden">
          {/* Card header */}
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-800/40">
            <span className="text-sm font-bold text-neutral-700 dark:text-neutral-200">Supplier Overview</span>
            <span className="text-xs font-bold text-neutral-400 dark:text-neutral-500 tabular-nums">
              {totalActive + totalInactive} total
            </span>
          </div>
          {/* Rows — one per supplier type */}
          <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {(Object.entries(TAB_CONFIG) as [SupplierType, typeof TAB_CONFIG[SupplierType]][]).map(([type, c]) => (
              <div key={type}
                onClick={() => { setActiveTab(type); }}
                className="flex items-center gap-4 px-5 py-4 hover:bg-neutral-50 dark:hover:bg-neutral-800/40 cursor-pointer transition-colors group">
                {/* Icon */}
                <div className={cn("w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0", c.bg, c.border)}>
                  <c.icon className={cn("w-4 h-4", c.color)} strokeWidth={1.8} />
                </div>
                {/* Label */}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-neutral-800 dark:text-white">{c.label}</p>
                  <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">{c.description}</p>
                </div>
                {/* Count + view link */}
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className={cn("text-2xl font-bold tabular-nums", c.color)}>
                    {counts[type] ?? 0}
                  </span>
                  <span className={cn("flex items-center gap-0.5 text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity", c.color)}>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg shadow-md p-5 flex items-center gap-4 flex-1">
            {/* Add icon button */}
            <button onClick={() => setShowAdd(true)}
              className="w-14 h-14 rounded-lg bg-[#EAB308]/10 border-2 border-dashed border-[#EAB308]/40 hover:bg-[#EAB308]/20 hover:border-[#EAB308]/70 flex items-center justify-center flex-shrink-0 transition-colors group">
              <div className="flex flex-col items-center">
                <User className="w-5 h-5 text-[#EAB308]" strokeWidth={1.8} />
                <Plus className="w-3 h-3 text-[#EAB308] -mt-0.5" strokeWidth={2.5} />
              </div>
            </button>
            {/* Content */}
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-neutral-400 dark:text-neutral-500 uppercase tracking-widest mb-1">Active Suppliers</p>
              <div className="flex items-end gap-2">
                <p className="text-4xl font-bold text-emerald-600 dark:text-emerald-400 leading-none">{totalActive}</p>
                <span className="text-xs text-neutral-400 dark:text-neutral-500 mb-1">suppliers</span>
              </div>
            </div>
            {/* Status dot */}
            <div className="flex-shrink-0">
              <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/40 px-2.5 py-1 rounded-lg">
          ...
          <div className="bg-neutral-900 dark:bg-neutral-950 rounded-lg shadow-md p-5 flex items-center gap-4 flex-1">
            {/* Icon */}
            <div className="w-14 h-14 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0">
              <UserX className="w-6 h-6 text-neutral-400" strokeWidth={1.8} />
            </div>
            {/* Content */}
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1">Inactive Suppliers</p>
              <div className="flex items-end gap-2">
                <p className="text-4xl font-bold text-white tabular-nums leading-none">{totalInactive}</p>
                <span className="text-xs text-neutral-500 mb-1">suppliers</span>
              </div>
              <p className="text-xs text-neutral-500 mt-1.5">Not participating in orders</p>
            </div>
            {/* Status dot */}
            <div className="flex-shrink-0">
              <span className="flex items-center gap-1.5 text-xs font-semibold text-red-400 bg-red-900/20 border border-red-900/40 px-2.5 py-1 rounded-lg">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {(Object.entries(TAB_CONFIG) as [SupplierType, typeof TAB_CONFIG[SupplierType]][]).map(([type, c]) => (
          <button key={type} onClick={() => setActiveTab(type)}
            className={cn(
              "flex items-center gap-2 px-4 py-2.5 rounded-lg border text-sm font-semibold transition-all whitespace-nowrap flex-shrink-0 shadow-sm",
              activeTab === type
                ? cn(c.activeBg, c.activeBorder, c.activeText, "shadow-md")
      ...
      {/* ─── LIST ────────────────────────────────────── */}
      <TabPanel key={activeTab} supplierType={activeTab} onSelect={setSelected} />

      {showAdd && (
        <SupplierFormModal supplierType={activeTab}
          onClose={() => {
            setShowAdd(false);
