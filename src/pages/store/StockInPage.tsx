...
const StockInPage = () => {
  ...
  const { data: recentMovements = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("stock_movements").select("*, store_items(name, item_code)").eq("movement_type", "in").order("created_at", { ascending: false }).limit(50);
      return data || [];
    },
  });
  ...
  const stockIn = useMutation({
    ...
    onSuccess: () => {
      ...
      setSelectedItem(""); setQty(""); setPurpose("");
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Stock IN</h1>
        ...
        <CardContent className="p-4 space-y-3">
          <div>
            ...
            <Select value={selectedItem} onValueChange={setSelectedItem}>
              ...
              <SelectContent>
                {items.map((i: any) => (
                  <SelectItem key={i.id} value={i.id}>{i.item_code} — {i.name} (Curr: {i.current_qty} {i.unit})</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>Purpose / Reason</Label><Input value={purpose} onChange={(e) => setPurpose(e.target.value)} placeholder="Procurement receipt, etc." /></div>
          </div>
          <Button className="w-full sm:w-auto" onClick={() => stockIn.mutate()} disabled={!selectedItem || !qty || stockIn.isPending}>
            <TrendingUp className="w-4 h-4 mr-2" />Record Stock IN
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">Recent Stock IN</h3>
            ...
            {recentMovements.map((m: any) => (
              <div key={m.id} className="flex flex-col sm:flex-row sm:items-center justify-between py-2 border-b border-border last:border-0 gap-1">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{(m as any).store_items?.item_code} • {m.purpose || "Manual entry"}</p>
                </div>
                <div className="flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => { setEditData({ id: m.id, quantity: m.quantity, purpose: m.purpose || "" }); setEditOpen(true); }}>
                    <Edit className="w-3 h-3" />
                  </Button>
                </div>
              </div>
            ))}
            {recentMovements.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No recent stock IN</p>}
          </div>
        </CardContent>
      </Card>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Edit Stock IN Entry</DialogTitle></DialogHeader>
          {editData && (
